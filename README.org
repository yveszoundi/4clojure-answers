...
#+begin_src clojure
  ...
  (defn tic-tac-toe-winner [boards]
    (let [magic-square [[8 1 6] [3 5 7] [4 9 2]]
          row-count (count boards)
          row-count-range (range row-count)
          dimension-winner (fn [row]
                             (case (reduce + row) 15 :o, 30 :x, nil))
          cell-to-num (fn [cell mapped-cell]
                        (case cell :e 0, :o mapped-cell, :x (* mapped-cell 2)))
      ...
      (let [num-matrix (map transform-row boards magic-square)
        ...
        (loop [winner nil, rows num-matrix-with-cols-and-diags]
          (if (or winner (empty? rows))
            winner
            (recur (dimension-winner (first rows))
                   (rest rows))))
...
#+begin_src clojure
  ...
  (defn smallest-common-num [& xs]
    (letfn
        (nav-skip [col cur max]
          (cond
           (= cur max) (nthnext col 1)
           
          :else (recur col (first (nthnext col)) max)))
      ...
      (if (= nb-cols 1)
        ...
        (loop [cols xs
               result -1
               lowest-elements #{}
               one-col-exhausted false
               all-visited false]

          (map )

          (recur (map #(nthnext %1 1)) cols)
