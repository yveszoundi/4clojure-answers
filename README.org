...
* INPROGRESS Problem 53
  ...
  #+BEGIN_SRC clojure
    ...
    (defn lcs
      ...
      (let [size (count col)
            compute-results (fn [i0 i1 results]
                              (if (> i1 i0)
                                (if-let [last-seq (last results)]
                                  (let [new-results (assoc results
                                                      (dec (count results))
                                                      (conj last-seq i1))]
                                    new-results)
                                  (conj results [i0 i1]))
        ...
        (if (<= size 1) []
            (loop [results[], idx 1]
              (if (= size idx)
                ...
                (let [prev (col (dec idx))
                      cur (col idx)
                      next-results (compute-results prev cur results)]
                  (recur next-results (inc idx))))))))
