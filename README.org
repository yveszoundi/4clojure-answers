    ...
    ([]
       (letfn [(add-prime? [candidate prime-set]
                 (let [narrowed-primes-set (reduce-primes-set candidate prime-set)]
                   (if (valid-candidate? candidate narrowed-primes-set)
                     candidate
               ...
               (reduce-primes-set [candidate primes-set]
                 (let [max-val (inc (long (Math/ceil (Math/sqrt candidate))))]
                   (take-while #(< %1 max-val) primes-set)))

               (valid-candidate? [current-candidate prime-set]
                 (not-any? #(zero? (mod current-candidate %1)) prime-set))
  ...
  #+BEGIN_SRC clojure
    ...
    (defn balanced-prime? [num]
      (letfn [(primes-list [n]
              ...
              (median [vec-of-3-items]
                (let [middle (vec-of-3-items 1)
                      lower-bound (vec-of-3-items 0)
                      upper-bound (vec-of-3-items 2)]
                  (/ (+ upper-bound lower-bound) 2)))]
        
        (let [n-primes 3
              primes-v (last-n-primes n-primes num)]
          (if (or (empty? primes-v)
                  (< (count primes-v) n-primes)
                  (not= (primes-v 1) num))
            false
            (= (median primes-v) num)))))
