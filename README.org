...
* Problem 53
  ...
  #+BEGIN_SRC clojure
    ...
    (and (= (lcs [1 0 1 2 3 0 4 5]) [0 1 2 3])
         (= (lcs [5 6 1 3 2 7]) [5 6])
         (= (lcs [2 3 3 4 5]) [3 4 5])
         (= (lcs [7 6 5 4]) []))
  #+END_SRC
...
#+begin_src clojure
  ...
  ;;
  ;; Not really fast but maybe "ok" for small numbers below 100000

  (defn prime-sieve [n]
    {:pre [(pos? n)]}
    (let [add-prime? (fn [x xs limit]
                       (let [max-val (inc (long (Math/ceil (Math/sqrt x))))]
                         (if (not-any? (fn [a]
                                         (zero? (mod x a)))
                                       (for [j xs :while (< j max-val)] j))
                           [(conj xs x) (dec limit)]
                           [xs limit])))]
      (lazy-seq
       (loop [i 3 , prime-xs [2], limit (dec n)]
         (if-not (pos? limit)
           prime-xs
           (let [prime-op-ctx (add-prime? i prime-xs limit)]
             (recur (+ i 2)
                    (prime-op-ctx 0)
                    (prime-op-ctx 1))))))))
...
#+begin_src clojure
  ...
  ;; TBD improve the first attempt which was training with loops and such
...
#+begin_src clojure
  ...
  (defn happy-num? [n]
    {:pre [(pos? n)]}
    (letfn [(digits-seq [x] ; order doesn't matter
              (when (pos? x)
                (cons (mod x 10) (digits-seq (long (/ x 10))))))
            (square-sum [xs]
              (long (reduce  #(+ %1 (Math/pow %2 2)) 0 xs)))]
      (loop [loop-detection #{}, i n]
        (let [sum (square-sum (digits-seq i))]
          (cond
           (= 1 sum) true
...
* Problem 120
  ...
  #+BEGIN_SRC clojure
    ...
    (defn sum-sqrt-digits [xs]
      (println "\n\n")
      (letfn [(digits [n]
                (lazy-seq
                 (if (> n 10)
                   (cons (long (mod n 10)) (digits (long (/ n 10))))
                   (cons n nil))))

              (sum-sqrt [col]
                (println "Col is:" col)
                (reduce #(+ %1 (Math/sqrt %2)) 0 col))]
        (reduce (fn [sum num]
                  (println "Num:" num
                           "Sum:" sum
                           "digits:" (digits num)
                           "sum-sqrt:" (sum-sqrt (digits num)))
                  (if (<= (long num) (long (sum-sqrt (digits num)) ))
                    (let [new-sum (inc sum)]
                      (println "FinalSum:" new-sum)
                      new-sum)
                    sum))
                0
                xs)))

    (and (= 8  (sum-sqrt-digits (range 10)))
         (= 19 (sum-sqrt-digits (range 30)))
         (= 50 (sum-sqrt-digits (range 100)))
         (= 50 (sum-sqrt-digits (range 1000))))
  #+END_SRC
...
* WAITING Problem 146
  ...
  #+BEGIN_SRC clojure
    ...
    ;;   "Flattens a map."
    ;;   [m]
    ;;   (reduce (fn step [col kv]
    ;;             (let [k  (first kv), v (last kv)]
    ;;               (if (map? v)
    ;;                 (do
    ;;                   (println "is a map:" "k:" k "v:" v))
    ;;                  (conj col (vector (reduce step [[k v]] v)))
    ;;                  (do
    ;;                    (println "Not a map" k)
    ;;                  (conj col [k v]))))
    ;;           []
