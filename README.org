...
#+begin_src clojure
  ...
  ;; TBD implement me
  (defn continuous-word-chain? [xs]
    (letfn [(word-vec [word] (into [] (seq word)))

            (pick-words [word1 word2]
              (if (> (count word2) (count word1))
                [word2 word1]
                [word1 word2]))
            ...
            (min-value
              "top, top-left, left"
              [ch-w1 ch-w2 ch-idx matrix col-idx row-idx]
              (cond
               (= ch-w1 ch-w2) 0
               :else (min   (inc ((new-matrix col-idx) (dec row-idx)))
                            (inc ((new-matrix (dec col-idx)) (dec row-idx)))
                            (inc ((new-matrix (dec col-idx)) row-idx)))))

            (compute-distance [word1 word2 matrix]
              (loop [row-idx 1,  max-row (count word2), m matrix]
                (if (= row-idx max-row)
                  (distance m)
                  (recur (inc row-idx)
                         max-row
                         (loop [col-idx 1, max-col-idx (count word1), m-mod m]
                           (if (= col-idx max-col-idx)
                             m-mod
                             (let [ch-w1 (word1 (dec row-idx))
                                   ch-w2 (word2 (dec col-idx))
                                   min-val (min-value (ch-w1 ch-w2 m-mod row-idx col-idx))]
      ...
      (let [col (into [] xs)
            words (pick-words (word-vec (col 0)) (word-vec (col 1)))
            word1 (first words), word2 (last words)
